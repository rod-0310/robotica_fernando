...
class Nodo2(Node):
    def __init__(self):
        super().__init__('nodo2')  # Nombre del nodo
        self.publisher_ = self.create_publisher(Int32, 'even', 10)  # Crear publicador en "numeros_impares"
        self.timer = self.create_timer(1.0, self.timer_callback)  # Temporizador cada 1s
        self.numero = 0  # Empezamos desde el primer número impar

        self.get_logger().info("Nodo2 par iniciado - Publicando números pares.")
